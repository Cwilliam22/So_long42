...
int	wall_check(t_game *game)
{
	...
	{
		if (game->grid[0][j] != '1')
			return (ft_printf("Error : wall_check1 fails"), 0);
		j++;
	}
	j = 0;
	while (game->grid[game->height - 1][j])
	{
		if (game->grid[game->height - 1][j] != '1')
			return (ft_printf("Error : wall_check2 fails"), 0);
		j++;
	}
	while (i < game->height - 1)
	{
		if (game->grid[i][0] != '1' || game->grid[i][game->length - 1] != '1')
			return (ft_printf("Error : wall_check3 fails"), 0);
		i++;
	}
	return (1);
}
...
int	ecp_check(t_game *game)
{
	...
	if (game->e != 1 || game->c < 1 || game->p != 1)
		return (ft_printf("Error : ecp_check fails"), 0);
	return (1);
}
	...
	while (game->grid[i])
	{
		...
		while (game->grid[i][j])
		{
			if (game->grid[i][j] != 'E' &&
				...
				game->grid[i][j] != '1' &&
				game->grid[i][j] != '0')
				return (ft_printf("Error : char_nvalid fails"), 0);
			j++;
		}
		i++;
	}
